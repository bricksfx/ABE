...
class deCrypto:
    ...
    def lagrangeCoeff(self, i, S, val):
        ...
        return int(
            fmod(reduce((lambda x, y: x * y), [int(fmod((j - val), gPair.p)) for j in S if j != i], 1), gPair.p)), int(
            reduce((lambda x, y: x * y), [j - i for j in S if j != i], 1))

    def decifraFoglia(self, E, D, x):
        if x.attribute[0] in E[0]:
            return int(
                gPair.e([y for xx, y in self.privateKey if xx == x.attribute[0]][0], E[2][E[0].index(x.attribute[0])]))
        else:
            return -1
    ...
    def decifraNodo(self, E, D, x):
        if x.children == []:
            ...
            if len(sxcompleto) < x.threshold:
                ...
                return int(fmod(reduce((lambda x, y: x + y), [
                    gPair.pot(gPair.radice(y, self.lagrangeCoeff(x, sprimex, 0)[1]),
                              self.lagrangeCoeff(x, sprimex, 0)[0]) for x, y in sx], 0) + gPair.p, gPair.p))
